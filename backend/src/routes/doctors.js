...
router.get('/', authenticate, async (req, res) => {
  try {
    ...
    console.error('Retrieve doctors list error:', error);
...
router.get('/stats', authenticate, async (req, res) => {
  try {
    ...
    // Run independent database aggregation queries concurrently using Promise.all()
    ...
    res.json({
      ...
      debugInfo: {
        ...
        notes: 'Loaded concurrently with Promise.all() for optimal performance.'
      }
    });
  } catch (error) {
    console.error('Retrieve stats error:', error);
